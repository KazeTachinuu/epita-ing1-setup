...
: "${PIE_RES:=1680x1050}"                            # gui window size (WxH)
...
cmd_vm() { ## boot the REAL PIE (QEMU/KVM) in a window; close the window to shut it down
    # needs the VM built into the nixpie-vm volume (see README maintainers)
    pkill -f "Xwayland $PIE_DISPLAY" 2>/dev/null && sleep 1 || true
    # no trap: Xwayland must outlive this script; closing its window ends
    # Xwayland, QEMU loses the display and exits, the container removes itself
    Xwayland "$PIE_DISPLAY" -ac -geometry "$PIE_RES" -decorate -host-grab 2>/dev/null &
    sleep 1
    docker rm -f pie-vm >/dev/null 2>&1 || true
    # the QMP socket lets `paste` type the host clipboard into the guest
    docker run -d --rm --name pie-vm --device /dev/kvm \
    ...
    ok "VM booting in its own window (close the window to shut it down)"
    info "Ctrl+Shift toggles the keyboard grab; paste host clipboard: $0 paste"
}
